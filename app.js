...
app.get("/tutorStudent", function(req, res){

  Inbox.find({}).sort({mssgSendTime:1}).exec(function(err, foundMsgs) {
    ...
    User.find().where('_id').in(userIds).exec(function(err, foundUsers) {
      ...
      res.render("tutorStudent", {foundMssgs: data});
    });
  });
});
...
function createPair(msg, userList) {
  ...
  for (i=0; i< userList.length; i++) {
    ...
    if (user._id == msg.senderUserID) {
      var pair = {msgObj: msg, userObj: user};
      return pair;
    }
  }
}
...
app.post("/signup", function(req, res){
  ...
  User.findOne({username: req.body.username}, function(err, foundUsers){
      if(foundUsers === null){
        User.register({username: req.body.username, age: req.body.age, tutor: isTutor, accountCreationTime: Date.now()}, req.body.password, function(err, user){
          if(err){
            console.log(err);
            res.redirect("/signup");
          }else{
